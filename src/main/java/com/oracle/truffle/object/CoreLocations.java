...
abstract class CoreLocations {
    ...
    static int getLocationOrdinal(CoreLocation loc) {
        ...
        if (internal instanceof FieldLocation) {
            return (isPrimitive ? -Integer.MAX_VALUE : 0) + ((FieldLocation) internal).getIndex();
        } else if (internal instanceof ArrayLocation) {
            return (isPrimitive ? -Integer.MAX_VALUE : 0) + MAX_DYNAMIC_FIELDS + ((ArrayLocation) internal).getIndex();
        } else {
